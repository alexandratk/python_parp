...
//			}
	...
	public static int[][] array(String filestr){
        String[] strNumb = filestr.split(System.lineSeparator());
        String[][] matrix = new String[strNumb.length][];
        for (int i = 0; i < strNumb.length; i++) {
        	matrix[i] = strNumb[i].split(" ");
        }
        int[][] arrNumb = new int[strNumb.length][matrix[0].length];
        for (int i = 0; i < strNumb.length; i++) {
        	for (int j = 0; j < matrix[i].length; j++) {
        		arrNumb[i][j] = Integer.parseInt(matrix[i][j]);
        	}
        }
        return arrNumb;
    }
